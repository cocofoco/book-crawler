...
# Cristian- Creo una lista vuota dove aggiungere tutti i miei book info
ListWithBooKInfo = []

# Cristian- Poi creo una classe dove specifico come vorrei fosse la mia "Scheda Libro"


class BookInformation:
    def __init__(self, Title, Author, Publisher, PublishingYear, LuogoDiPubblicazione, Edizione, Dimensions, Weight, Legatura, Collana, Soggetti, NumeroPagine, Volumi, Description):
        self.Title = Title
        self.Author = Author
        self.Publisher = Publisher
        self.PublishingYear = PublishingYear
        self.LuogoDiPubblicazione = LuogoDiPubblicazione
...
def search_maremagnum():
    ...
    number_of_results = int(soup_maremagnum.find(
        'section', id='inside-search').label.span.text)
    ...
    for book in soup_maremagnum.findAll('li', class_='note', limit=25):
        ...
        list_links_bookpage.append(website_maremagnum +
                                   finalpart_link_bookpage.attrs['href'])
...
"""
    ...
    def search_luogo_pubblicazione(self):
        luogo_pubblicazione_location = self.find(
            'b', string=re.compile('Luogo di pubblicazione'))
...
# Cristian- Prima aggiungo alla lista una nuova "Scheda libro" e poi specifico come vorrei fosse questa "scheda libro"
# Cristian- Nello specifico al posto di speficare le varie funzioni prima le spefico nella creazione del BookInfo

        ListWithBooKInfo.append(BookInformation(
            # Ad esempio qui specifico che il titolo deve essere creato usando quello che viene trovato da details.find(ecc)...
            Title=details.find('h1', itemprop='name').text,
            Author=details.find('h2', itemprop='author').text,
            Publisher=details.find('span', itemprop='publisher').text,
            Collana=details.find('b', string=re.compile('Collana').text,
            # .....

# Cristian- Ovviamente questo non e finito e magari questo approccio non ti piace o non funziona in questo caso.
# Cristian- Pero usando l'approccio "Object Oriented" ti viene anche piu facile in futuro utilizzare i dati che hai ottenuto. Ad esempio se vuoi metterli in una tabella exel.
                        ...
                        try:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    try:
                                        book_luogo_pubblicazione = search_luogo_pubblicazione(
                                            details)
                                    except (AttributeError, NameError):
                                        book_luogo_pubblicazione = None
                                    finally:
                                        try:
                                            book_note_bibliografiche = search_note_bibliografiche(
                                                details)
                                        except (AttributeError, NameError):
                                            book_note_bibliografiche = None
                                        finally:
                                            try:
                                                book_volumi = search_volumi(
                                                    details)
                                            except (AttributeError, NameError):
                                                book_volumi = None
                                            finally:
                                                try:
                                                    book_edizione = search_edizione(
                                                        details)
                                                except (AttributeError, NameError):
                                                    book_edizione = None
                                                finally:
                                                    try:
                                                        book_num_pagine = search_num_pagine(
                                                            details)
                                                    except (AttributeError, NameError):
